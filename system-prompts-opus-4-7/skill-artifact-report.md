...
description: >-
  Bundled artifact-report skill — Create a long-form report artifact —
  typographic document with a masthead, table of contents, structured sections,
  and an optional appendix. Use when the user asks for a report, analysis,
...
- Lead with what matters most; push details, methodology, and raw data to the end or the appendix.
- Plain language, short sentences; define each term of art on first use and expand abbreviations. Cut anything that doesn't change what the reader knows or decides.
- Mark how certain each claim is — separate what was measured from what is inferred from what is speculation, rather than presenting all three in one voice.
