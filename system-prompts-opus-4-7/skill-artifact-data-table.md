...
description: >-
  Bundled artifact-data-table skill — Create an interactive data-table artifact
  — a sortable, filterable table for exploring a tabular dataset. Use when the
  user wants to browse, sort, or filter rows of data (a CSV, a list of records,
...
2. Copy it as your starting point. Replace each \`<!-- SLOT: ... -->\` marker with real content — the comment inside each slot describes what goes there. Each slot also carries placeholder text; replace that too — removing the markers alone leaves the placeholders in the published page.
3. Before publishing, confirm no \`SLOT\` markers and no placeholder text remain.
4. Publish the filled HTML with the \`Artifact\` tool.

**Creation only.** When editing an existing data-table artifact, work with its current HTML directly — don't re-apply this template.

## Slots

| Slot | What to fill in |
| --- | --- |
| \`TITLE\` | Plain-text page title, e.g. "Product catalog". Appears **twice** — the \`<title>\` element near the top (names the browser tab and the artifact) and the visible \`<h1>\` in the header. Fill both. |
...
- Values in \`"num"\` columns must be JSON numbers, not strings — \`1234.5\`, never \`"1,234.50"\` or \`"$1,234.50"\`. Strip currency symbols and thousands separators; put the unit in the column label (e.g. "Amount (USD)"). A non-numeric value in a \`"num"\` column is shown as-is but skips number formatting and sorts to the end.
...
- Inside JSON string values, escape \`</\` as \`<\\/\` and \`<!--\` as \`<\\u0021--\` (both are valid JSON and parse back to the original text). Unescaped, \`</script\` terminates the script block early — breaking the table and rendering the rest of the value as live HTML — and \`<!--\` opens an HTML comment-like state with similar effects.
...
- The filter input matches substrings across all text columns; numeric columns are excluded. A filter that matches nothing shows a built-in "No rows match" message — don't add your own.
