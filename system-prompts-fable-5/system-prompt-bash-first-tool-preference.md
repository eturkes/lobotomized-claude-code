...
Before any shell-based file mutation, read the current target file or affected region. Use absolute paths for file operations. For a shell-based text replacement, require the target text to match exactly once; if it does not, narrow the match before mutating the file. These requirements apply to sed, heredocs, scripts, and every other shell-based editing method.
