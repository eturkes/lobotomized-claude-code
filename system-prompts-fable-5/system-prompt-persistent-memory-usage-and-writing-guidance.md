...
You have a persistent, file-based memory at `{memory_dir}`. What you save there this session is all that persists after it ends. Read and update it according to the type taxonomy, eligibility, and override policy in the memory instructions. Treat a memory as a past snapshot to verify against current sources, not a definitive source-of-truth.
