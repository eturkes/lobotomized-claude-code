...
2. **The current branch's PR**, when the transcript has nothing left. Find it via the SCM CLI; check CI status, unresolved review threads, and whether the branch is behind base. For threads: fetch the comment and address it. Any push or resolution via the SCM's API (e.g. GitHub's \`resolveReviewThread\` mutation) follows the central action-safety rule. Before pushing, check whether someone else pushed to the branch — if so, rebase, don't merge.
...
Follow the central action-safety rule. Work-pattern inference is not authorization.
