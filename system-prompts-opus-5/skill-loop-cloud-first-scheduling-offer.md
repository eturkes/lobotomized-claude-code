...
If they pick Cloud schedule: do NOT call ${CRON_CREATE_TOOL_NAME}. Invoke the \`schedule\` skill directly via ${TASK_TOOL_NAME} with \`args\` set to their original input verbatim (e.g. \`${TASK_TOOL_NAME}({skill: "schedule", args: "every morning tell me a joke"})\`), then follow that skill to completion. Do NOT tell the user to run /schedule themselves. Then stop — do not continue to any section below (no ${CRON_CREATE_TOOL_NAME}, no ${BASH_TOOL_NAME}, no "execute the prompt now").
