...
description: >-
  Bundled artifact-dashboard skill — Create a dashboard artifact — KPI tiles, a
  primary time-series chart, and a breakdown table. Use when the user asks for a
...
Base styling and reusable components for an operational dashboard: KPI tile cards, a spec-driven line-chart renderer, and table styles, in a default layout (KPI row, primary chart, breakdown table).
...
3. Then make the dashboard fit the data and the ask: add charts (the \`dataviz\` skill helps design richer or additional visualizations), reorder or drop sections, extend the layout grid — building on the card system, renderer, and table styles so the result stays coherent.
...
- The chart slot takes a JSON spec, not markup — you emit data plus a few knobs and the template's \`renderChart()\` owns the pixels.
- **Fill every placeholder with real data, or remove its whole section** — KPI numbers, table rows, and the chart spec's zeroed "REPLACE ME" series. The footer's data source and generation date come from the conversation or are omitted.
- **No time dimension?** Don't force a time axis onto data that has none. Three paths: use \`"type": "bar"\` or \`"type": "donut"\` when that shape fits (deterministic, preferred for standard shapes); hand-draw your own SVG or HTML chart for a shape the spec doesn't cover or full visual control (the \`dataviz\` skill helps design it — reuse the card chrome and palette); or drop the chart section and lead with the KPI tiles and breakdown table. Prefer \`"line"\` for a trend: it is the only spec type the page can still draw if the published page's chart runtime is unavailable — a hand-drawn SVG chart has no such dependency.
