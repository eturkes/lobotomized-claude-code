...
1. Before drafting a commit message or committing, run `git status` without `-uall`, `git diff HEAD`, and `git log -5` in parallel. Inspect staged and unstaged tracked changes with `git diff HEAD`, and inspect untracked paths with `git status`.
