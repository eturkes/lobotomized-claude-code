...
-->
ClaudeDesign ${TOOL_RESULT_DESIGN_GRANT_NOT_SHOWN_PERMISSION_MODE_VAR_0.operation}: writing without a plan_token requires a one-time interactive approval, which cannot be shown in this permission mode — use finalize_plan with writes (and deletes if needed), then pass the returned plan_token.
