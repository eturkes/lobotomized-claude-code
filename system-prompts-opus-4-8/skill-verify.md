...
The scope is what you're verifying — usually a diff, sometimes just "does X work." In a git repo, establish the full range (a branch may be many commits, or still uncommitted):
...
for GUI/TUI, code block for library/API; omit for build/types-only>
...
**Evidence has to reach the reader.** A file path is only evidence if the reader can open it. If \`SendUserFile\` is in your toolset, you're on a remote surface where they can't — send the screenshots and recordings with it and name them in the report. Without it, keep the evidence that matters inline: pane captures and response bodies travel in the report; a bare path only works when the reader shares your filesystem.
...
- **BLOCKED** — couldn't reach a state where the change is observable (build broke, missing dep, handle wouldn't come up). Not a verdict on the change. Before calling it blocked, enumerate the skills along the touched subtree — environment-specific unlocks (headless runners, login helpers) usually live there. Say exactly where it stopped + a \`/run-skill-generator\` prompt.
